...
class SinceStoppedView extends WatchUi.SimpleDataField {
    ...
    function initialize() {
        ...
        self._lastMovingTime = Time.now();

        // jittery speed readings are when the device thinks it's moving because GPS sometimes
        // provides different values while stationary. Observations show fake speed readings may
        // happen in bursts of two or three at a time, then fall back to zero. So we take
        // _jitterThreshold consecutive speed readings as a sign that we are geniunely moving.
        self._jitterDetector = 0;
        self._jitterThreshold = 20;

        // the threshold is 5 minutes, after which we are considered stopped
    ...
    function getDisplayText(info as Activity.Info, timeNow as Time.Moment) as String {
        ...
        // jitter detector: 0 < _jitterDetector < 10
        //
        ...
        // if there is no jitter, then we are moving
        if (currentSpeed > 0 && self._jitterDetector == 0) {
            ...
            // we are not moving or paused, but we may have jittered recently
            ...
            // are we are over the non-moving threshold (ie stopped at a cafe)?
